...
export default class Navigation extends Skyact.SkyactComponent {
  ...
  render() {
    const mainScreenIcon =
      this.state.activePage !== routes.mainScreen
        ? mainScreen
        : mainScreenActive;
    const citySavedIcon =
      this.state.activePage !== routes.citySaved ? citySaved : citySavedActive;
    const menuIcon = this.state.activePage !== routes.menu ? menu : menuActive;
